...
$(function(){
    ...
    if(localStorage['event'].length!=0){
        ...
        for(var i=2;i<6;i++){
            $('#sidebar ul').append('<li><img src="_img/'+evnt_name+'/'+i+'.png"/></li>');
        }
        $('#video source').attr('src',"_img/"+evnt_name+"/video.mp4");
    }
...
function draw_viz(){
    ...
    var dataset =[]
    d3.text("_data/HVC636853.csv", function(text) {
            ...
            svg.append("g")
                .attr("class", "x axis")
                .attr("transform", "translate(55.5,15)")
                .style("fill", 'white')
                .style('font-size','10')
                .call(xAxis);
            // X axis label
